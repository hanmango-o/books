...
BFS는 너비 탐색이기에 현재 노드들이 연결되어 있는지 확인할 수 있습니다.

문제 조건에서 `연결되어 있는~` 과 같은 표현이 사용될 경우 그 연결 여부를 BFS를 통해  확인할 수 있습니다.
...
BFS 탐색을 진행할 경우 부모 -> 자식 방향으로 탐색을 진행하게 됩니다.

이때, 문제의 조건에 따라 BFS 적용 시 탐색할 수 없는 영역이 존재할 수 있습니다.

예를 들어 그래프가 아래와 같을 때, {1, 2, 3, 4} 노드 조합을 탐색하고 싶다고 가정합니다.
...
1번 노드에서 BFS 탐색을 한다고 가정했을 때, 위와 같은 {1, 2, 3, 4} 조합은 2, 3, 4번 조합의 depth 가 동일하기에 탐색이 불가능합니다.

이에 그래프 탐색 시 주어진 조건에 맞는 경우가 탐색 가능한지 여부를 판단하는 것이 중요합니다.

{% hint style="info" %}
이와 같은 상황에서 그래프(BFS, DFS) 탐색을 적용할 때에는 탐색이 가능 여부를 판단해야 합니다.\
\
즉,  주어진 노드가 부모-자식 관계로 어떻게 맺어지는 지 살펴보는 것이 중요합니다.
{% endhint %}
