...
이를 통해 해결할 수 있는 대표적인 문제는정렬되지 않은 리스트에서 최대/최소/중앙값 찾기 입니다.

특히정렬되지 않은 경우, 모든 환경(데이터)이 설정된 후 최대/최소/중앙값을 찾는 상황에서도 우선 순위 큐가 효율적입니다.

또한 환경 변화가 발생하는 상황에서 최대/최소/중앙값을 지속적으로 찾아야 하는 상황에서는 우선 순위 큐를 활용하는 것이 가장 좋은 방법입니다.
