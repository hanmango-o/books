...
이런 경우 DAT 배열을 활용하여 해결할 수 있습니다.

```cpp
int result = N; // 전체 Union 개수
int cnt = 0; // 단일 원소 집합을 제외한 Union 개수
void Union(int a, int b) {
    int pa = Find(a);
    int pb = Find(b);
    if(pa == pb) return;
...
`parent[i] == i` 를 통해 단일 집합을 제외하고, DAT 배열을 통해 중복 확인하는 집합이 없도록 합니다.

이후 개수를 + 1 해주면서 **2개 이상의 원소로 이루어진 집합**의 개수를 확인할 수 있습니다.
