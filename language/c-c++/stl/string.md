...
{% hint style="info" %}
string::npos 는 -1 과 동일합니다.\
즉, string::npos == -1 는 true 입니다.
{% endhint %}
...
size_t index = str.find(value); // [1] 시작 지점 찾기
if(index != string::npos) cnt++; // [2] 존재 여부 카운팅하기
...
문자열의 비교는 기존 operator 를 활용하여 쉽게 가능합니다.
...
특정 문자열의 인덱스 범위에 따른 삭제와 특정 인덱스를 기준으로 삽입하는 함수는 아래와 같습니다.

```cpp
str.erase(index); // index~ 모든 문자 삭제
str.erase(start, count); // start 부터 count개수 만큼 삭제(start + count - 1 인덱스까지 삭제)

str.insert(index, value) // index에 value 문자열 삽입
str.insert(index, value, count) // index에 value 문자열의 0번 인덱스부터 count 개수 만큼의 부분문자열 삽입
...
str.insert(2, "good"); // hegoodllo, index 바로 직전에 삽입
str.insert(2, "good", 2); // hegollo
...
* 문자열은 `char` 배열과 `string` 라이브러리를 사용하여 표현할 수 있습니다.
* 문자열의 길이는 char 배열의 경우 `'\0'`를 찾는 방법과, string의 경우 `length()` 를 사용하여 알 수 있습니다.
* string 의 각 문자는 배열처럼 접근 가능하며, operator의 사용 역시 가능합니다.
* 부분 문자열 추출은 `str.substr(시작 지점, 추출 문자열 길이)` 로 가능합니다.
* 문자열 포함 여부 판단은 `str.find(찾고자 하는 문자열)` 또는 `str.find(찾고자 하는 문자열, 시작 지점)` 으로 가능합니다.
  * 이때 값을 찾지 못했으면 `string::npos`를 반환하며, 이는 -1 과 동일합니다.
* 문자열 수정은 `str.replace(시작 지점, 삭제할 문자열 크기, 삽입할 문자열)` 로 가능합니다.
* 문자열 삽입은 `str.insert(시작 지점, 삽입할  문자열)` 또는 `str.insert(시작 지점, 삽입할 문자열, 삽입할 문자열의 크기)` 로 가능합니다.
* 문자열 삭제는 `str.erase(시작 지점)` 또는 `str.erase(시작 지점, 삭제할 문자열의 크기)` 로 가능합니다.
